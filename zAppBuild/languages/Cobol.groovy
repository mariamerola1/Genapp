...
sortedList.each { buildFile ->
	...
	if ( props.userBuild) {
             def memLower = member.toLowerCase()
             def memUpper = member.toUpperCase()
		     def testfile = new File("${props.workspace}/${props.zunitDirs}/${member}.json")
             def testCase
             def playBackFile
             println("${props.workspace}/${props.zunitDirs}/${member}.json")
             if (!testfile.exists()) {
            	 testfile = new File("${props.workspace}/${props.zunitDirs}/${memLower}.json")
            	 println("Checking ${props.workspace}/${props.zunitDirs}/${memLower}.json")
             }
             if (!testfile.exists()) {
            	 testfile = new File("${props.workspace}/${props.zunitDirs}/${memUpper}.json")
            	 println("Checking ${props.workspace}/${props.zunitDirs}/${memUpper}.json")
             }
             if (testfile.exists()) {
...
def retrieveTestparms(File testfile) {
    ...
    if (playBackFile.contains('<HLQ>')) {
       println("can find")
    }
  
    // change HLQ
   	playBackFile = playBackFile.replaceAll("<HLQ>",,"IBMUSER")
   	def test="&"
    println(test + ':test')
