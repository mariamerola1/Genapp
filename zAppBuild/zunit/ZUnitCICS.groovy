...
println("dbbhome environment setting: ${dbb_home}")
if (!dbb_home) dbb_home = '/var/dbb/1.0.6'
def dbb_conf = dbb_home + '/conf'
	
println("dbbhome default: ${dbb_home}")
...
int rc = exec.text(jcl).confDir(dbb_conf).execute()

/* Check the return code from the submitted job */
//def maxRC = exec.getMaxRC()
println("rc:" + rc)
sleep(20000)
println("maxRC:"+ exec.getMaxRC())

if ((exec.maxRC == 'CC 0000') || (exec.maxRC == 'CC 0004'))
    println "The JCL Job ${exec.submittedJobId} completed successfully"
else { 
    println "The JCL Job ${exec.submittedJobId} completed with Max-RC: ${exec.maxRC}"
    props.error = "true"
    System.exit(1)
}
